...
--
--  Fixed some issues with soft reset and with error detection.
--
-- END_CHANGELOG
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
...
--  issues.  
--
--  For MPMC to LLink clock ratios greater than 1, added register to break
--  long timing path.
--
-- @END_CHANGELOG
-------------------------------------------------------------------------------
...
entity sdma is
    generic (
        ...
        C_PI_DATA_WIDTH         : integer range 64 to 64    := 64;
        C_PI_BE_WIDTH           : integer range 8 to 8      := 8;
...
-------------------------------------------------------------------------------
DATA_CROSS : if C_PI2LL_CLK_RATIO > 1 generate
        ...
        begin
            if(PI_Clk'EVENT and PI_Clk='1')then
                if(pi_rst = '1')then
                    ...
                    PI_RdFIFO_Pop   <= PI_RdFIFO_Pop_i   and not clk_pi_enable;
                end if;
            end if;
...
DATA_NO_CROSS : if C_PI2LL_CLK_RATIO = 1 generate
...
EMPTY_CROSS : if C_PI2LL_CLK_RATIO > 1 generate
        ...
        begin
            if(LLink_Clk'EVENT and LLink_Clk='1')then
                if(LLink_Rst = '1')then
                  ...
                  PI_RdFIFO_Empty_i <= PI_RdFIFO_Empty;
                end if;
            end if;
        end process RDFIFO_EMPTY;
end generate EMPTY_CROSS;

EMPTY_NO_CROSS : if C_PI2LL_CLK_RATIO = 1 generate
    PI_RdFIFO_Empty_i <= PI_RdFIFO_Empty;
...
-------------------------------------------------------------------------------  
-------------------------------------------------------------------------------  
DMA_CONTROL_I : entity sdma_v1_00_b.sdma_cntl
    generic map(
...
-------------------------------------------------------------------------------  
-------------------------------------------------------------------------------  
DMA_DATA_I : entity sdma_v1_00_b.sdma_datapath
    generic map(
        C_INSTANTIATE_TIMER_TX              => INSTANTIATE_TIMER_TX                 ,
        C_INSTANTIATE_TIMER_RX              => INSTANTIATE_TIMER_RX                 ,
        C_PI_RDDATA_DELAY                   => C_PI_RDDATA_DELAY                    ,
...
I_IPIF_BLK : entity plbv46_slave_single_v1_00_a.plbv46_slave_single
    generic map (
